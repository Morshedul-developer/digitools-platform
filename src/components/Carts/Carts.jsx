    ...
    <div className="max-w-7xl mx-auto mb-20">
      <div className="border border-gray-200 p-8 rounded-2xl shadow-xl">
        ...
        <div className="space-y-4 mb-4">
          {cards.map((card) => (
            <CartCard key={card.id} card={card} handleRemove={handleRemove} />
          ))}
        </div>
        {/* empty cart */}
        {cards.length === 0 ? (
          ...
          <>
            ...
            <button
              onClick={handleCarts}
              className="btn rounded-full w-full border-none bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white hover:from-[#9514FA] hover:to-[#4F39F6] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-[#4F39F6]/40 active:translate-y-0 active:scale-95 hover:text-white/85 font-bold"
            >
              Proceed to Checkout
            </button>
          </>
